...
player_status() {
    timeout 1.5 playerctl --player="$1" status 2>/dev/null || printf 'Stopped'
}

select_player() {
    local players player status first="" paused=""

    players=$(timeout 1.5 playerctl -l 2>/dev/null) || return 1
...
PLAYER_NAME=$(select_player || true)
pc() {
    [ -n "$PLAYER_NAME" ] || return 1
    timeout 1.5 playerctl --player="$PLAYER_NAME" "$@"
}
...
if [ "$STATUS" = "Playing" ] || [ "$STATUS" = "Paused" ]; then
    ...
    # --- 3. GET INFO ---
    rawUrl=$(pc metadata mpris:artUrl 2>/dev/null)
    title=$(pc metadata xesam:title 2>/dev/null)
    artist=$(pc metadata xesam:artist 2>/dev/null)
    ...
    # --- 5. TIMING ---
    len_micro=$(pc metadata mpris:length 2>/dev/null)
    ...
    if [ "$STATUS" = "Playing" ]; then
        pos_micro=$(pc metadata --format '{{position}}' 2>/dev/null)
    ...
    # --- 6. DEVICE INFO ---
    player_raw=$(pc status -f "{{playerName}}" 2>/dev/null | head -n 1)
