...
Item {
    ...
    // IMPERATIVE_COLOR_ENGINE_BEGIN
    property string colorEngine: "matugen"
    property alias wallpaperResizeMode: searchState.resizeMode

    Process {
        id: colorEngineLoadProc
        command: ["bash", "-c", "cat ~/.config/imperative/color-engine 2>/dev/null || printf matugen"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let value = this.text.trim();
                if (value === "pywal" || value === "matugen")
                    window.colorEngine = value;
            }
        }
    }

    function setColorEngine(engine) {
        if (engine !== "pywal" && engine !== "matugen") return;
        window.colorEngine = engine;
        Quickshell.execDetached([
            "bash", "-c",
            "mkdir -p ~/.config/imperative && printf '%s\\n' '" + engine + "' > ~/.config/imperative/color-engine"
        ]);
    }
    // IMPERATIVE_COLOR_ENGINE_END
    ...
    Process {
        id: monitorProc
        command: ["sh", "-c", "export PATH=$PATH:/usr/bin:/usr/local/bin:/run/current-system/sw/bin && niri msg --json outputs 2>/dev/null || echo '[]'"]
        running: false
    ...
    function loadMonitors() {
        console.log("[MonitorSync] Starting native Niri output process...");
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        let engineScript = Qt.resolvedUrl("color_engine_apply.sh").toString();
        
        if (reloadScript.startsWith("file://")) {
            reloadScript = decodeURIComponent(reloadScript.substring(7));
        }
        if (engineScript.startsWith("file://")) {
            engineScript = decodeURIComponent(engineScript.substring(7));
        }
        ...
        const escResize = escapeBash(window.wallpaperResizeMode);
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    export ENGINE_SCRIPT="${escapeBash(engineScript)}"
                    export TARGET_MONITORS="${escOutputs}"
                    export RESIZE_MODE="${escResize}"
                    ...
                    if [ "$TARGET_MONITORS" = "all" ]; then
                        awww img "$DEST_FILE" --resize "$RESIZE_MODE" --transition-type ${randomTransition} --transition-pos 0.5,0.5 --transition-fps 144 --transition-duration 1 >> ${logFile} 2>&1 &
                    else
                        awww img -o "$TARGET_MONITORS" "$DEST_FILE" --resize "$RESIZE_MODE" --transition-type ${randomTransition} --transition-pos 0.5,0.5 --transition-fps 144 --transition-duration 1 >> ${logFile} 2>&1 &
                    fi
                    
                    ( bash "$ENGINE_SCRIPT" "$FINAL_THUMB" "$RELOAD_SCRIPT" || true ) &
                `;
        ...
        const escEngine = escapeBash(engineScript);
        ...
        if (isVideo) {
            wallpaperCmd = `
                ...
                if [ "${escOutputs}" = "all" ]; then
                    mpvpaper -o 'loop --no-audio --hwdec=no --profile=high-quality --video-sync=display-resample --interpolation --tscale=oversample' '*' "${escOriginal}" >> ${logFile} 2>&1 &
                else
                    IFS=',' read -ra MON_ARR <<< "${escOutputs}"
                    for mon in "\${MON_ARR[@]}"; do
                        mpvpaper -o 'loop --no-audio --hwdec=no --profile=high-quality --video-sync=display-resample --interpolation --tscale=oversample' "\$mon" "${escOriginal}" >> ${logFile} 2>&1 &
                    done
                fi
            `;
        } else {
            wallpaperCmd = `
                ...
                if [ "${escOutputs}" = "all" ]; then
                    awww img "${escOriginal}" --resize "${escResize}" --transition-type ${randomTransition} --transition-pos 0.5,0.5 --transition-fps 144 --transition-duration 1 >> ${logFile} 2>&1 &
                else
                    awww img -o "${escOutputs}" "${escOriginal}" --resize "${escResize}" --transition-type ${randomTransition} --transition-pos 0.5,0.5 --transition-fps 144 --transition-duration 1 >> ${logFile} 2>&1 &
                fi
            `;
        }
        ...
        const fullScript = `
            ...
            ${wallpaperCmd}
            ( bash "${escEngine}" "${escThumb}" "${escReload}" || true ) &
        `;
    ...
    Rectangle {
        ...
        Row {
            ...
            // IMPERATIVE_COLOR_ENGINE_TOGGLE_BEGIN
            Rectangle {
                id: colorEngineToggle
                width: window.s(72)
                height: window.s(36)
                radius: window.s(10)
                anchors.verticalCenter: parent.verticalCenter
                color: Qt.rgba(_theme.surface0.r, _theme.surface0.g, _theme.surface0.b, 0.82)
                ...
                Rectangle {
                    ...
                    height: parent.height
                    x: window.colorEngine === "pywal" ? 0 : parent.width / 2
                    radius: window.s(9)
                    color: _theme.text
                ...
                Row {
                    ...
                    Repeater {
                        model: [
                            { label: "P", engine: "pywal" },
                            { label: "M", engine: "matugen" }
                        ]
                        delegate: Item {
                            width: colorEngineToggle.width / 2
                            height: colorEngineToggle.height
                            Text {
                                anchors.centerIn: parent
                                text: modelData.label
                                color: window.colorEngine === modelData.engine ? _theme.base : _theme.text
                                font.family: "JetBrains Mono"
                                font.pixelSize: window.s(14)
                                font.bold: true
                            }
                            MouseArea {
                                ...
                                enabled: !window.isApplying
                                cursorShape: Qt.PointingHandCursor
                                onClicked: window.setColorEngine(modelData.engine)
                            }
                        }
                    }
                }
            }
            // IMPERATIVE_COLOR_ENGINE_TOGGLE_END

            Rectangle {
                id: resizeModePicker
                property bool menuOpen: false
                property bool isHovered: resizeModeMouse.containsMouse

                width: window.s(102)
                height: window.s(36)
                radius: window.s(10)
                anchors.verticalCenter: parent.verticalCenter
                color: isHovered || menuOpen
                    ? Qt.rgba(_theme.surface1.r, _theme.surface1.g, _theme.surface1.b, 0.92)
                    : Qt.rgba(_theme.surface0.r, _theme.surface0.g, _theme.surface0.b, 0.82)
                border.color: menuOpen ? _theme.text : _theme.surface1
                border.width: 1
                clip: false
                z: 100
                ...
                Row {
                    ...
                    Text {
                        text: resizeModePicker.displayName(window.wallpaperResizeMode)
                        color: _theme.text
                        font.family: "JetBrains Mono"
                        font.pixelSize: window.s(12)
                        font.bold: true
                    ...
                    Text {
                        text: resizeModePicker.menuOpen ? "󰅃" : "󰅀"
                        color: Qt.rgba(_theme.text.r, _theme.text.g, _theme.text.b, 0.72)
                ...
                MouseArea {
                    ...
                    enabled: !window.isApplying
                    cursorShape: Qt.PointingHandCursor
                    onClicked: resizeModePicker.menuOpen = !resizeModePicker.menuOpen
                }

                Rectangle {
                    id: resizeModeMenu
                    visible: resizeModePicker.menuOpen
                    width: resizeModePicker.width
                    height: resizeModeColumn.implicitHeight + window.s(8)
                    anchors.top: parent.bottom
                    anchors.topMargin: window.s(7)
                    anchors.horizontalCenter: parent.horizontalCenter
                    radius: window.s(10)
                    color: Qt.rgba(_theme.mantle.r, _theme.mantle.g, _theme.mantle.b, 0.98)
                    border.color: _theme.surface2
                    border.width: 1
                    z: 101
                    ...
                    Column {
                        ...
                        Repeater {
                            model: [
                                { label: "Zoom", value: "crop" },
                                { label: "Stretch", value: "stretch" },
                                { label: "Fit", value: "fit" }
                            ]

                            delegate: Rectangle {
                                width: resizeModeMenu.width
                                height: window.s(32)
                                radius: window.s(7)
                                color: window.wallpaperResizeMode === modelData.value
                                    ? Qt.rgba(_theme.surface2.r, _theme.surface2.g, _theme.surface2.b, 0.95)
                                    : resizeOptionMouse.containsMouse
                                        ? Qt.rgba(_theme.surface1.r, _theme.surface1.g, _theme.surface1.b, 0.78)
                                        : "transparent"
                                ...
                                Text {
                                    ...
                                    color: window.wallpaperResizeMode === modelData.value
                                        ? _theme.text
                                        : Qt.rgba(_theme.text.r, _theme.text.g, _theme.text.b, 0.78)
                                    font.family: "JetBrains Mono"
                                    font.pixelSize: window.s(12)
                                    font.bold: window.wallpaperResizeMode === modelData.value
                                }

                                MouseArea {
                                    id: resizeOptionMouse
                                    anchors.fill: parent
                                    hoverEnabled: true
                                    enabled: !window.isApplying
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: {
                                        window.wallpaperResizeMode = modelData.value;
                                        resizeModePicker.menuOpen = false;
                                    }
                                }
                            }
                        }
