...
Item {
    ...
    // STATE & CONFIG
    // -------------------------------------------------------------------------
    readonly property string scriptsDir: Quickshell.env("HOME") + "/.config/quickshell/volume"
    ...
    function updateHeroData() {
        let targetModel = outputsModel;
        if (window.activeTab === "inputs") targetModel = inputsModel;
        if (window.activeTab === "apps") targetModel = appsModel;

        if (window.activeTab === "apps") {
            if (targetModel.count === 0) {
                window.activeId = "";
                window.activeName = "No Applications";
                window.activeDesc = "No active application streams";
                window.activeIcon = "audio-card";
            ...
            for (let i = 0; i < targetModel.count; i++) {
                if (targetModel.get(i).id === window.selectedAppId) {
                    appIndex = i;
                    break;
                }
            }

            let app = targetModel.get(appIndex);
            window.selectedAppId = app.id;
            window.activeId = app.id;
            window.activeName = app.description;
            window.activeDesc = app.name;
            window.activeIcon = app.icon;
            if (!window.draggingMaster) {
                window.activeVol = app.volume;
                window.activeMute = app.mute;
            }
            return;
        }
        ...
        if (!foundDefault && targetModel.count > 0) {
            ...
            window.activeId = "";
            window.activeName = window.activeTab === "apps" ? "No Applications" : "No Device";
            window.activeDesc = window.activeTab === "apps" ? "No active application streams" : "";
            window.activeIcon = "audio-card";
            if (!window.draggingMaster) {
                window.activeVol = 0;
                window.activeMute = false;
                    ...
                    RowLayout {
                        ...
                        Item {
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    if (window.activeTab === "apps" && window.activeId === "") return;
                                    let type = "sink";
                                    let id = "@DEFAULT@";
                                    ...
                                    Quickshell.execDetached(["bash", window.scriptsDir + "/audio_control.sh", "toggle-mute", type, id]);
                            ...
                            RowLayout {
                                ...
                                Item {
                                    ...
                                    Timer {
                                        ...
                                        onTriggered: {
                                            if (targetPct >= 0) {
                                                if (window.activeTab === "apps" && window.activeId === "") {
                                                    targetPct = -1;
                                                    return;
                                                }
                                                let type = "sink";
                                                ...
                                                Quickshell.execDetached(["bash", window.scriptsDir + "/audio_control.sh", "set-volume", type, id, targetPct]);
                                                targetPct = -1;
                                            }
                                        }
                                    }
                ...
                Item {
                    ...
                    ListView {
                        ...
                        Item {
                            ...
                            ColumnLayout {
                                ...
                                Text {
                                    ...
                                    text: window.activeTab === "apps" ? "No active applications" : "No active streams"
                                }
                            }
                        }
                        ...
                        delegate: Rectangle {
                            ...
                            property bool isSelectedApp: window.activeTab === "apps" && model.id === window.selectedAppId
                            ...
                            color: isActiveNode ? window.tabColor : (isHovered || isSelectedApp ? "#0affffff" : "#05ffffff")
                            border.color: (isActiveNode || isSelectedApp) ? window.tabColor : "#1affffff"
                            border.width: (isActiveNode || isSelectedApp) ? 2 : 1
                            Behavior on border.color { ColorAnimation { duration: 300 } }
                            Behavior on color { ColorAnimation { duration: 300 } }
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    if (window.activeTab === "apps") {
                                        window.selectedAppId = model.id;
                                        window.updateHeroData();
