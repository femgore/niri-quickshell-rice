...
Item {
    ...
    readonly property string niriScriptsDir: homeDir + "/.config/niri/scripts"
    readonly property string qsScriptsDir: quickshellDir
    readonly property string cacheDir: paths.cacheDir
    
    readonly property string settingsJsonPath: quickshellDir + "/settings.json"
    ...
    property int initialWorkspaceCount: 5
    ...
    function applyMonitors() {
        ...
        if (monitorsModel.count === 1) {
            ...
            Quickshell.execDetached(["notify-send", "Display Update", "Monitor editing is disabled in the portable Niri build. Edit ~/.config/niri/config.kdl instead."]);
        } else {
            let rects = [];
            for (let i = 0; i < monitorsModel.count; i++) {
                let m = monitorsModel.get(i);
    ...
    Process {
        id: _displayPoller
        command: ["bash", "-c", "niri msg --json outputs 2>/dev/null || echo '[]'"]
        running: false
