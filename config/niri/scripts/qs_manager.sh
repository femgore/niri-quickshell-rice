...
# GLOBAL VARS
# -----------------------------------------------------------------------------
SCRIPTS_DIR="$HOME/.config/quickshell"
SHELL_QML_PATH="$SCRIPTS_DIR/Shell.qml"
SHELL_LAUNCHER="$SCRIPTS_DIR/launch-igpu.sh"
...
if [[ "${ACTION:-}" =~ ^[0-9]+$ ]]; then
    if [[ -n "${NIRI_SOCKET:-}" ]] || pgrep -x niri >/dev/null; then
        ...
        notify-send "Niri" "Workspace switching requires a running Niri session." \
            >/dev/null 2>&1 || true
    fi
...
SRC_DIR="${WALLPAPER_DIR:-${srcdir:-$HOME/.config/Wallpapers}}"
...
if ! pgrep -f "quickshell.*Shell.qml" >/dev/null; then
    "$SHELL_LAUNCHER" -p "$SHELL_QML_PATH" >/dev/null 2>&1 &
    disown
fi
